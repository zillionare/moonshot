...
def monthly_factor_backtest(
    ...
    for i in range(len(month_ends) - 1):
        ...
        if quantiles is not None:
            ...
            # bins不能为None，因为前面已经检查过了
            assert bins is not None, "bins cannot be None when quantiles is None"
            month_data["group"] = (
                pd.cut(
...
def analyze_long_short_spread(monthly_returns: pd.DataFrame) -> "pd.Series[float]":
    """
    分析多空价差（最高分组 - 最低分组）
