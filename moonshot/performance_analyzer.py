...
class PerformanceAnalyzer:
    ...
    def __init__(
        ...
        self.benchmark_returns = (
            self._validate_returns(benchmark_returns, "基准收益率")
    ...
    def _validate_returns(self, returns: pd.Series, name: str) -> pd.Series:
        ...
        if returns.index.inferred_type not in [
            "datetime64",
            "timedelta64",
            "date",
            "period",
    ...
    def _setup_proxy_methods(self) -> None:
        ...
        self.cagr = lambda: qs.stats.cagr(
            self.strategy_returns, periods=self.annual_periods
        )
        self.alpha = (
            lambda: qs.stats.alpha(self.strategy_returns, self.benchmark_returns)
            if self.benchmark_returns is not None
            else None
        )
        self.beta = (
            lambda: qs.stats.volatility(self.strategy_returns)
            if self.benchmark_returns is not None
            else None
        )
        ...
        def generate_html_report(
            output: str = "strategy_report.html", title: str = "策略绩效报告"
        ):
            """生成HTML格式的完整报告"""
            qs.reports.html(
        ...
        def generate_tear_sheet():
            ...
            qs.reports.basic(
                self.strategy_returns, benchmark=self.benchmark_returns, title="策略绩效分析"
            )
            ...
            metrics.update(
                {
                    "阿尔法(α)": self.alpha(),
                    "贝塔(β)": self.beta(),
                    "信息比率": qs.stats.information_ratio(
